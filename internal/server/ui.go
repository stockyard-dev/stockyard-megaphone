...
async function load(){var r=await fetch(A+'/incidents').then(function(r){return r.json()});items=r.incidents||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.title||'').toLowerCase().includes(q)||(i.status||'').toLowerCase().includes(q)||(i.severity||'').toLowerCase().includes(q)||(i.affected_services||'').toLowerCase().includes(q)});
...
if(i.status)h+='<div class="item-sub">'+esc(i.status)+'</div>';
...
if(i.affected_services)h+='<span>'+esc(i.affected_services)+'</span>';
if(i.updated_by)h+='<span>'+esc(i.updated_by)+'</span>';
if(i.resolved_at)h+='<span>'+esc(i.resolved_at)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
if(i.message)h+='<div class="item-notes">'+esc((i.message||'').substring(0,200))+'</div>';
h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/incidents/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{title:"",status:"",severity:"",affected_services:"",message:"",updated_by:"",resolved_at:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' MEGAPHONE</h2>';
...
h+='<div class="fr"><label>Affected Services</label><input id="f-affected-services" value="'+esc(i.affected_services)+'"></div>';
h+='<div class="fr"><label>Message</label><textarea id="f-message" rows="3">'+esc(i.message)+'</textarea></div>';
h+='<div class="fr"><label>Updated By</label><input id="f-updated-by" value="'+esc(i.updated_by)+'"></div>';
h+='<div class="fr"><label>Resolved At</label><input id="f-resolved-at" type="date" value="'+esc(i.resolved_at)+'"></div>';
...
async function submit(){var body={title:document.getElementById('f-title').value.trim(),status:document.getElementById('f-status').value.trim(),severity:document.getElementById('f-severity').value.trim(),affected_services:document.getElementById('f-affected-services').value.trim(),message:document.getElementById('f-message').value.trim(),updated_by:document.getElementById('f-updated-by').value.trim(),resolved_at:document.getElementById('f-resolved-at').value.trim()};
if(editId){await fetch(A+'/incidents/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/incidents',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
